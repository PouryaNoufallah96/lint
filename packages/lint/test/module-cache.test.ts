...
test("a large barrel shares its complete exports and dependencies across files", () => {
  ...
  for (let i = 0; i < 200; i++) {
    expected.add(
      write(`control-${i}.tsx`, `export const Control${i} = () => null`)
    )
  ...
  for (let i = 0; i < 200; i++) {
    ...
    expect(
      definingExportOf(
        "./index",
        `Control${i}`,
        path.join(root, `page-${i}.tsx`),
        deps
      )
    ).toEqual({
      file: path.join(root, `control-${i}.tsx`),
      name: `Control${i}`,
    })
    expect(deps).toEqual(new Set([barrel, path.join(root, `control-${i}.tsx`)]))
  }
})
...
test("named resolution agrees with full exports through aliases, defaults, and cycles", () => {
  ...
  for (const file of Object.keys(sources)) {
    ...
    for (const name of [...expected.keys(), "default", "Unknown"]) {
      expect(
        definingExportOf(`./${file}`, name, path.join(root, "page.tsx")),
        `${file}: ${name}`
...
test("a closure computed from a stale mtime memo is dropped once the mtime is seen", () => {
  ...
  write("badge.tsx", "export const Badge = () => null")
  const barrel = write(
    "widgets.ts",
  ...
  mtimeOf(barrel)
  mtimeOf(path.join(root, "button.tsx"))
...
test("a default export wrapped in memo or forwardRef is the component inside", () => {
  const page = path.join(root, "page.tsx")
  const button = write(
    "button.tsx",
    'import { memo } from "react"\nfunction Button() { return null }\nexport default memo(Button)'
  )
  expect(definingExportOf("./button", "default", page)).toEqual({
    file: button,
    name: "Button",
  })
  const input = write(
    "input.tsx",
    'import * as React from "react"\nexport default React.forwardRef(function Input() { return null })'
  )
  expect(definingExportOf("./input", "default", page)).toEqual({
    file: input,
    name: "Input",
  })
})
