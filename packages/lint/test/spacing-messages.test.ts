...
const saveButton = `import { SaveButton } from "@/components/save-button"`
      ...
      invalid: [
        {
          filename: PAGE,
          options: [layout],
          code: `${button}\nexport const A = () => <Button className="p-4 mt-4">Go</Button>`,
          errors: [
            {
              message:
                /^"p-4" is not allowed on <Button>: <Button> owns its spacing\. Use a size \(default, xs, sm, lg, icon, icon-xs, icon-sm, icon-lg\), or margin here or gap on the parent for space around it\. Add a size in test\/fixtures\/project\/components\/ui\/button\.tsx only if the design explicitly calls for one\.$/,
            },
          ],
        },
        {
          filename: PAGE,
          options: [layout],
          code: `${button}\nexport const A = () => <Button className="gap-2">Go</Button>`,
    ...
    tester.run("no-restyle", rule, {
      ...
      invalid: [
        // No variant definition in the file, no enclosing container.
        {
          filename: PAGE,
          options: [layout],
          code: `${card}\nexport const A = () => <CardTitle className="pb-2">Hi</CardTitle>`,
          errors: [
            {
              message:
        ...
        // A variant axis, but no size axis.
        {
          filename: PAGE,
          options: [layout],
          code: `${field}\nexport const A = () => <FieldLabel className="px-2">Hi</FieldLabel>`,
          errors: [
            {
              message:
    ...
    tester.run("no-restyle", rule, {
      ...
      invalid: [
        // The direct parent accepts spacing: it replaces "the parent".
        {
          filename: PAGE,
          options,
          code: `${cardHeader}\nexport const A = () => <CardHeader><CardTitle className="pb-2">Hi</CardTitle></CardHeader>`,
          errors: [
            {
              message: new RegExp(
        ...
        // Further up: the parent line stays and the container is added.
        {
          filename: PAGE,
          options,
          code: `${button}\n${card}\nexport const A = () => <CardContent><div><Button className="p-4">Go</Button></div></CardContent>`,
          errors: [
            {
              message: new RegExp(
        ...
        // Fragments and expression containers are not layout parents.
        {
          filename: PAGE,
          options,
          code: `${button}\n${card}\nexport const A = ({ on }: { on: boolean }) => <CardContent>{on && <><Button className="p-4">Go</Button></>}</CardContent>`,
          errors: [
            {
              message: new RegExp(
        ...
        // An enclosing element that does not accept the class is skipped.
        {
          filename: PAGE,
          options,
          code: `${field}\nexport const A = () => <Field><FieldLabel className="px-2">Hi</FieldLabel></Field>`,
        ...
        // A wrapper of an open container is named as written.
        {
          filename: PAGE,
          options,
          code: `${button}\nimport { Panel } from "@/components/panel"\nexport const A = () => <Panel><Button className="p-4">Go</Button></Panel>`,
          errors: [
            {
              message: new RegExp(
      ...
      invalid: [
        {
          filename: PAGE,
          options: [layout],
          code: `${saveButton}\nexport const A = () => <SaveButton className="py-1">Go</SaveButton>`,
          errors: [
            {
              message:
                /^"py-1" is not allowed on <SaveButton>: <SaveButton> forwards className to <Button>, which owns its spacing\. Use a size \(default, .*\), or margin here or gap on the parent for space around it\. Add a size in .*button\.tsx/,
            },
          ],
        },
      ],
      ...
      invalid: [
        {
          ...
          code: `${button}\nexport const A = () => <Button className="p-4 m-2">Go</Button>`,
          errors: [
            {
              message: /, or gap on the parent for space around it\. Add/,
            },
    ...
    tester.run("no-restyle", rule, {
      ...
      invalid: [
        ...
        // index order, and not the component being restyled.
        {
          filename: PAGE,
          options,
          code: `${card}\nexport const A = () => <CardTitle className="p-4">Hi</CardTitle>`,
          errors: [
            {
              message: new RegExp(
      ...
      invalid: [
        {
          ...
          code: `import { Chip } from "@acme/kit"\nexport const A = () => <Chip className="px-3">Hi</Chip>`,
          errors: [
            {
              message:
...
describe("size axis reader", () => {
  test("reads the size axis and nothing else", async () => {
    ...
    const buttonFile = path.join(PROJECT, "components/ui/button.tsx")
    ...
    const fieldFile = path.join(PROJECT, "components/ui/field.tsx")
