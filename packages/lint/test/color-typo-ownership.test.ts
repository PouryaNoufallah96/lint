...
describe.skipIf(!oracle)("a typo the grammar files under a color", () => {
  const code = `export const A = () => <div className="text-smal bg-primry bg-brand" />`
  ...
  test("no-raw-colors alone leaves it and keeps the token findings", () => {
    createTester().run("no-raw-colors", noRawColors as any, {
      valid: [
        {
          filename: PAGE,
          code: `export const A = () => <div className="text-smal" />`,
        },
      ],
      invalid: [
        {
  ...
  test("with both rules on, each class is reported once", () => {
    expect(
      ...
      { ruleId: "shadcn/no-unknown-classes", messageId: "unknownClassSuggest" },
      { ruleId: "shadcn/no-raw-colors", messageId: "undeclaredTokenTypo" },
      { ruleId: "shadcn/no-raw-colors", messageId: "undeclaredToken" },
    ])
  })
})
