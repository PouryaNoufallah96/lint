...
test("layout and denied diagnostics name the policy's list", () => {
  createTester().run("no-restyle", noRestyle as any, {
    valid: [
      {
        filename: PAGE,
        code: `${button}\nexport const A = () => <Button className="mt-4 w-full">Go</Button>`,
        options: [layout],
      },
    ],
    invalid: [
...
test("the rule's own message speaks for the top-level policy", () => {
  createTester().run("no-restyle", noRestyle as any, {
    ...
    invalid: [
      ...
      {
        filename: PAGE,
        code: `${button}\nexport const A = () => <><Button className="p-6">Go</Button><Card className="p-6">Hi</Card></>`,
        options: [
          {
            allow: ["layout"],
  ...
  test("get their own words in the rule", () => {
    createTester().run("no-restyle", noRestyle as any, {
      ...
      invalid: [
        {
          filename: PAGE,
          code: `${button}\nexport const A = () => <Button className="flex-cols">Go</Button>`,
